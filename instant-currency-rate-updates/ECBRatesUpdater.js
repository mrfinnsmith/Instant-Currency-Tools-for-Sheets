function updateECBRates() {
  ...
  const targetTime = new Date(cetTime.toISOString().split('T')[0] + 'T06:45:00Z');

  if (cetTime < targetTime) {
    console.log('It is not yet 16:45 CET/CEST. Exiting.');
    return;
  }
  // Originally got all currencies from Frankfurter app
...
function updateCacheAndFirestore(baseCurrency, targetCurrency, data) {
  ...
  console.log(PropertiesService.getScriptProperties().getProperty('firebaseToken'));
